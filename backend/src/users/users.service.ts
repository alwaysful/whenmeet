...
import { CreateUserDto, AuthProvider } from './dto/create-user.dto';
...
export class UsersService {
  ...
  async findById(id: number) {
    ...
    if (user.deletedAt) throw new NotFoundException('삭제된 사용자입니다.');
    return user;
  }
  ...
  async updateProfileImage(id: number, imageUrl: string) {
    ...
    return this.prisma.user.update({
      where: { id },
      data: { profileImage: imageUrl, updatedAt: new Date() },
    });
  }
